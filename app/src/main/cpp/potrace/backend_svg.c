...
int my_page_svg(FILE *fout, potrace_path_t *plist, imginfo_t *imginfo) {
    if (!fout || !plist || !imginfo) return 1;

    double w = imginfo->width;
    double h = imginfo->height;

    // --- Header ---
    fprintf(fout, "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 %g %g\">\n", w, h);
    fprintf(fout, "  <path fill=\"none\" stroke=\"black\" stroke-width=\"1\" d=\"\n");
    ...
    for (potrace_path_t *p = plist; p; p = p->next) {
        ...
        fprintf(fout, "    Z\n\n"); // close each subpath and separate by a blank line
    }
